...
# A) Hard-sample soft weighting
LIP_SYNC_HARD_MODE="${LIP_SYNC_HARD_MODE:-clamp}"
LIP_SYNC_HARD_CAP="${LIP_SYNC_HARD_CAP:-2.0}"
LIP_SYNC_HARD_MIN_WEIGHT="${LIP_SYNC_HARD_MIN_WEIGHT:-0.2}"
LEGACY_DETACH="${LEGACY_DETACH:-0}"

# B) Timing-aware delay loss
LIP_SYNC_USE_DELAY_AWARE="${LIP_SYNC_USE_DELAY_AWARE:-1}"
LIP_SYNC_MAX_SHIFT="${LIP_SYNC_MAX_SHIFT:-3}"
LIP_SYNC_DELAY_LAMBDA="${LIP_SYNC_DELAY_LAMBDA:-0.05}"
LIP_SYNC_DELAY_MODE="${LIP_SYNC_DELAY_MODE:-best_shift_penalty}"

# C) Supervision window length (regenerate .npy when changing)
LIP_SYNC_NUM_FRAMES="${LIP_SYNC_NUM_FRAMES:-16}"

# D) Lip landmark loss (reuses existing lmk array)
USE_LIP_LANDMARK_LOSS="${USE_LIP_LANDMARK_LOSS:-0}"
LIP_LANDMARK_LAMBDA="${LIP_LANDMARK_LAMBDA:-0.02}"

# E) Per-loss warmup schedules
LIP_SYNC_LAMBDA1_WARMUP="${LIP_SYNC_LAMBDA1_WARMUP:-10}"
LIP_SYNC_DELAY_WARMUP="${LIP_SYNC_DELAY_WARMUP:-10}"
LIP_LANDMARK_WARMUP="${LIP_LANDMARK_WARMUP:-15}"
...
if [ "$USE_LIP_SYNC" -eq 1 ]; then
  echo "   Lip-sync     : ENABLED  λ1=$LIP_SYNC_LAMBDA1  λ2=$LIP_SYNC_LAMBDA2"
  echo "   Lip frames   : $LIP_SYNC_NUM_FRAMES  hard=$LIP_SYNC_HARD_MODE"
  echo "   Delay-aware  : enabled=$LIP_SYNC_USE_DELAY_AWARE  shift=±$LIP_SYNC_MAX_SHIFT  λ=$LIP_SYNC_DELAY_LAMBDA"
  echo "   Lmk loss     : enabled=$USE_LIP_LANDMARK_LOSS  λ=$LIP_LANDMARK_LAMBDA"
else
  echo "   Lip-sync     : disabled"
fi
echo ""
echo "═══════════════════════════════════════════════════════════════"
echo -e "${RESET}"
...
python scripts/preload_train_data_to_pkl.py \
    ...
    $([ "$USE_LIP_SYNC" -eq 1 ] && echo "--use_lip_sync_loss")
...
if [ "$NUM_GPUS" -gt 1 ]; then
    ...
    accelerate launch \
        ...
        --lip_sync_warmup_epochs ${LIP_SYNC_WARMUP} \
        --lip_sync_num_frames ${LIP_SYNC_NUM_FRAMES} \
        --lip_sync_hard_mode ${LIP_SYNC_HARD_MODE} \
        --lip_sync_hard_cap ${LIP_SYNC_HARD_CAP} \
        --lip_sync_hard_min_weight ${LIP_SYNC_HARD_MIN_WEIGHT} \
        --lip_sync_delay_lambda ${LIP_SYNC_DELAY_LAMBDA} \
        --lip_sync_max_shift ${LIP_SYNC_MAX_SHIFT} \
